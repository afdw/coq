...
let assoc_var s ist =
  let v = Id.ObservableMap.find (Names.Id.of_string s) ist.lfun in
...
let assoc_flags ist : tauto_flags =
  let Val.Dyn (tag, v) = Id.ObservableMap.find (Names.Id.of_string "tauto_flags") ist.lfun in
...
let with_flags flags _ ist =
  ...
  let ist = { ist with lfun = Id.ObservableMap.add x.CAst.v arg ist.lfun } in
...
let find_cut _ ist =
  let k = Id.ObservableMap.find (Names.Id.of_string "k") ist.lfun in
