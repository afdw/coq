...
let populate_late_arg_atomic late_arg tac =
  populate_glob_late_arg late_arg (Some (GenArg (Glbwit wit_value, Taccoerce.in_gen (topwit wit_atomic_tactic) tac)))
...
let populate_current_late_arg_atomic ist tac =
  let current_late_arg = TacStore.get ist.extra f_current_late_arg |> Option.default top_late_arg in
...
let pr_closure env sigma ist body =
  ...
  in
  let pp_iargs = v 0 (prlist_with_sep pr_sep pr_iarg (Id.Map.bindings (ist |> Id.TracedMap.forget))) in
...
let push_trace call ist =
  if is_traced () then match TacStore.get ist.extra f_trace with
  | None -> [call], [ist.lfun |> Id.TracedMap.forget]
  | Some (trace, varmaps) -> call :: trace, (ist.lfun |> Id.TracedMap.forget) :: varmaps
  else [],[]
...
let extend_values_with_bindings (ln,lm) lfun =
  ...
  let accu = Id.TracedMap.map value_of_ident (ln |> Id.TracedMap.remember) in
...
let try_interp_ltac_var coerce ist env {loc;v=id} =
  let v = Id.TracedMap.find id ist.lfun |> clone in
...
let interp_int_as_list ist = function
  | ArgVar ({v=id} as locid) ->
      (try coerce_to_int_list (Id.TracedMap.find id ist.lfun)
...
let interp_hyp_list_as_list ist env sigma ({loc;v=id} as x) =
  try coerce_to_hyp_list env sigma (Id.TracedMap.find id ist.lfun)
...
let interp_fresh_id ist env sigma l =
  ...
  | Some l -> l
  in
  let avoid = extract_ids ids (ist.lfun |> Id.TracedMap.forget) avoid in
  let id =
...
let interp_constr_in_compound_list inj_fun dest_fun interp_fun ist env sigma l =
  let try_expand_ltac_var sigma x =
    ...
    | GVar id ->
      let v = Id.TracedMap.find id ist.lfun in
...
let interp_message_ftactic ist l =
  ...
  l |> Ftactic.List.map (function
    ...
    | MsgIdent {loc; v = id} ->
      match ist.lfun |> Id.TracedMap.find_opt id with
      | None ->
        Ftactic.lift (
...
and interp_intro_pattern_list_as_list ist env sigma = function
  | [{loc;v=IntroNaming (IntroIdentifier id)}] as l ->
      (try coerce_to_intro_pattern_list ?loc sigma (Id.TracedMap.find id ist.lfun |> clone)
...
let tag_print_gen lfun k t =
  ...
  let old_r = lfun |> Id.Map.map (fun (_, r) -> !r) in
  lfun |> Id.Map.iter (fun id (_, r) ->
    r := !r |> GenRef.observe (fun _ ->
  ...
  lfun |> Id.Map.iter (fun id (_, r) -> r := old_r |> Id.Map.find id);
  Proofview.Trace.tag_tactic
    k
    (fun () ->
      let lfun = lfun |> Id.Map.mapi (fun id (x, _) -> (x, ref (GenRef.const (!used |> Id.Set.mem id)))) in
...
let f_last_lfun : value Id.TracedMap.t Evd.Store.field = Evd.Store.field "last_lfun"
...
let rec val_interp_ftactic deferred_id ist ?(appl = UnnamedAppl) (tac : glob_tactic_expr) : TaggedVal.t Ftactic.t =
      ...
      Proofview.Trace.tag_deferred_contents deferred_id (
        ...
        tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacDelay")) (
          ...
          Ftactic.return (
            TaggedVal.make
              deferred_id
              (of_tacvalue (VFun (UnnamedAppl, extract_trace ist, extract_loc ist, ist.lfun |> Id.Map.map (fun (x, r) -> (x, ref (!r))), [], tac)))
          )
        )
      ) in
  ...
  | DebugOn lev ->
    ...
    in
    Tactic_debug.debug_prompt lev tac eval (ist.lfun |> Id.TracedMap.forget) (TacStore.get ist.extra f_trace)
  | _ ->
    aux ist >>= fun v -> Ftactic.return (name_vfun appl v)
...
and eval_tactic_ist ist tac : unit Proofview.tactic =
  ...
  match tac.v with
  | TacAtom t ->
    tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacAtom")) @@
    ...
    if len1 = len2 then
      ...
      tag_print ist (Proofview_monad.Info.Alias (Pptactic.pr_alias_key s)) (
        ...
        let tac_f =
          ...
          let lfun = List.fold_right2 Id.TracedMap.add s_interp.alias_args args_interp ist.lfun in
...
and interp_ltac_reference_ftactic ?loc' mustbetac deferred_id ist r : TaggedVal.t Ftactic.t =
  ...
  | ArgVar {loc; v = id} ->
      ...
      Proofview.Trace.tag_deferred_contents deferred_id (
        let v =
          ist.lfun |> Id.TracedMap.find_opt id |> Option.map clone |> Option.default (in_gen (topwit wit_hyp) id) in
  ...
  | ArgArg (loc, r) ->
      Proofview.tclProofInfo [@ocaml.warning "-3"] >>= fun (_name, poly) ->
      let ids = extract_ids [] (ist.lfun |> Id.TracedMap.forget) Id.Set.empty in
  ...
  match to_tacvalue fv with
    ...
    | (VFun(appl,trace,_,olfun,(_::_ as var),body)
      |VFun(appl,trace,_,olfun,([] as var),
        ...
        let fold accu (id, v) = Id.TracedMap.add id v accu in
  ...
  Proofview.Trace.tag_deferred_contents deferred_id (
    ...
    tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacLetIn")) (
      ...
      let fold accu ({v = na}, b) =
        ...
        Name.fold_right (fun id -> Id.TracedMap.add id v) na accu in
  ...
  | (name, body) :: defs ->
    Proofview.Trace.tag_deferred_contents deferred_id (
      ...
      tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacLetIn")) (
        ...
        fold body_interp.deferred_id {ist with lfun = Name.fold_right (fun id -> Id.TracedMap.add id body_interp.v) name.v ist.lfun} defs
      )
    ) in
...
and interp_match_success deferred_id ist { Tactic_matching.subst ; context ; terms ; lhs } =
  Proofview.tclProofInfo [@ocaml.warning "-3"] >>= fun (_name, poly) ->
  let (>>=) = Ftactic.bind in
  let lctxt = Id.TracedMap.map interp_context (context |> Id.TracedMap.remember) in
  let hyp_subst = Id.TracedMap.map Value.of_constr (terms |> Id.TracedMap.remember) in
...
and interp_atomic ist tac : unit Proofview.tactic =
  ...
  | TacIntroPattern (ev,l) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        let sigma = project gl in
        let l' = interp_intro_pattern_list_as_list ist env sigma l in
        populate_current_late_arg_atomic ist (TacIntroPattern (ev, l')) <*>
  ...
  | TacElim (ev,(keep,cb),cbo) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        let tac =
          populate_current_late_arg_atomic ist (TacElim (ev, (keep, cb), cbo)) <*>
          tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacElim")) @@
            Tactics.elim ev keep cb cbo in
  ...
  | TacCase (ev,(keep,cb)) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        let sigma, cb = interp_open_constr_with_bindings ist env sigma cb in
        let tac =
          populate_current_late_arg_atomic ist (TacCase (ev, (keep, cb))) <*>
          tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacCase")) @@
            Tactics.general_case_analysis ev keep cb in
        Tacticals.tclWITHHOLES ev tac sigma
      end
  | TacMutualFix (id,n,l) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        in
        let tac =
          populate_current_late_arg_atomic ist (TacMutualFix (id, n, l_interp)) <*>
          tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacMutualFix")) @@
          Tactics.mutual_fix (interp_ident ist env sigma id) n l_interp 0 in
  ...
  | TacMutualCofix (id,l) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        in
        let tac =
          populate_current_late_arg_atomic ist (TacMutualCofix (id, l_interp)) <*>
          tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacMutualCofix")) @@
          Tactics.mutual_cofix (interp_ident ist env sigma id) l_interp 0 in
        Tacticals.tclTHEN (Proofview.Unsafe.tclEVARS sigma) tac
      end
  | TacAssert (ev,b,t,ipat,c) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        let tac =
          populate_current_late_arg_atomic ist (TacAssert (ev, b, t, ipat', c)) <*>
          tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacAssert")) @@
          Tactics.forward b tac ipat' c in
        Tacticals.tclWITHHOLES ev tac sigma
      end
  | TacGeneralize cl ->
    Proofview.Goal.enter begin fun gl ->
      ...
      let tac =
        populate_current_late_arg_atomic ist (TacGeneralize cl) <*>
        tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacGeneralize")) @@
        Generalize.generalize_gen cl in
      Tacticals.tclWITHHOLES false tac sigma
    end
  | TacLetTac (ev,na,c,clp,b,eqpat) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        (* We try to fully-typecheck the term *)
          ...
          let let_tac =
            populate_current_late_arg_atomic ist (TacLetTac (ev, na, c_interp, clp, b, eqpat)) <*>
        ...
        (* We try to keep the pattern structure as much as possible *)
          ...
          let tac =
            populate_current_late_arg_atomic ist (TacLetTac (ev, na, c, clp, b, eqpat)) <*>
            tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacLetTac")) @@
            let_pat_tac b (interp_name ist env sigma na) (Some sigma,c) clp eqpat in
          Tacticals.tclWITHHOLES ev tac sigma'
      end
  ...
  | TacInductionDestruct (isrec,ev,(l,el)) ->
      ...
      Proofview.Goal.enter begin fun gl ->
        ...
        let l' = l |> List.map (fun (c, (ipato, ipats), cls) -> (c, (ipato, ipats |> Option.map (fun x -> ArgArg x)), cls)) in
        let tac =
          populate_current_late_arg_atomic ist (TacInductionDestruct (isrec, ev, (l', el))) <*>
          tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacInductionDestruct")) @@
          Induction.induction_destruct isrec ev (l,el) in
        Tacticals.tclTHEN (Proofview.Unsafe.tclEVARS sigma) tac
      end
  ...
  | TacReduce (r,cl) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        let tac =
          populate_current_late_arg_atomic ist (TacReduce (r_interp, cl)) <*>
          tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacReduce")) @@
          Tactics.reduce r_interp cl in
        Tacticals.tclTHEN (Proofview.Unsafe.tclEVARS sigma) tac
      end
  ...
  | TacInversion (DepInversion (k,c,ids),hyp) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        let tac =
          populate_current_late_arg_atomic ist (TacInversion (DepInversion (k, c_interp, ids_interp |> Option.map (fun x -> ArgArg x)), hyp)) <*>
          tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacInversion")) @@
          Inv.dinv k c_interp ids_interp dqhyps in
        Tacticals.tclWITHHOLES false tac sigma
      end
  | TacInversion (NonDepInversion (k,idl,ids),hyp) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        let dqhyps = interp_declared_or_quantified_hypothesis ist env sigma hyp in
        let ids_interp = interp_or_and_intro_pattern_option ist env sigma ids in
        populate_current_late_arg_atomic ist (TacInversion (NonDepInversion (k, hyps, ids_interp |> Option.map (fun x -> ArgArg x)), hyp)) <*>
        tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacInversion")) @@
  ...
  | TacInversion (InversionUsing (c,idl),hyp) ->
      Proofview.Goal.enter begin fun gl ->
        ...
        let hyps = interp_hyp_list ist env sigma idl in
        let tac =
          populate_current_late_arg_atomic ist (TacInversion (InversionUsing (c_interp, hyps), hyp)) <*>
          tag_print ist (Proofview_monad.Info.Builtin (Pp.str "TacInversion")) @@
          Leminv.lemInv_clause dqhyps c_interp hyps in
        Tacticals.tclTHEN (Proofview.Unsafe.tclEVARS sigma) tac
      end
...
module Value = struct
  ...
  let apply_expr f args =
    let fold arg (i, vars, lfun) =
      ...
      (succ i, x :: vars, Id.TracedMap.add id arg lfun)
    in
    let (_, args, lfun) = List.fold_right fold args (0, [], Id.Map.empty) in
    let lfun = Id.TracedMap.add (Id.of_string "F") f lfun in
...
let interp_destruction_arg' deferred_id ist c =
  ...
  Proofview.Trace.tag_deferred_contents deferred_id (
    Ftactic.enter (fun gl ->
      let v = interp_destruction_arg ist gl c in
      populate_current_late_arg ist (CAst.make (TacArg (TacGeneric (None, GenArg (glbwit wit_value, Taccoerce.in_gen (topwit wit_destruction_arg) v))))) <*>
      tag_print ist (Proofview_monad.Info.Builtin (Pp.str "destruction_arg")) @@
        Ftactic.lift Proofview.Trace.new_deferred_placeholder >>= fun deferred_id ->
        Ftactic.return (Proofview.Tagged.make deferred_id v)
    )
  )
...
let () =
  register_interp0 wit_int_or_var (fun deferred_id ist n ->
    Proofview.Trace.tag_deferred_contents deferred_id (
      let v = interp_int_or_var ist n in
      populate_current_late_arg ist (CAst.make (TacArg (TacGeneric (None, GenArg (glbwit wit_int_or_var, ArgArg v))))) <*>
      tag_print ist (Proofview_monad.Info.Builtin (Pp.str "int_or_var")) (
        Proofview.Trace.new_deferred_placeholder >>= fun deferred_id ->
        Ftactic.return (Proofview.Tagged.make deferred_id v)
      )
    )
  );
  register_interp0 wit_nat_or_var (fun deferred_id ist n ->
    Proofview.Trace.tag_deferred_contents deferred_id (
      let v = interp_int_or_var ist n in
      populate_current_late_arg ist (CAst.make (TacArg (TacGeneric (None, GenArg (glbwit wit_nat_or_var, ArgArg v))))) <*>
      tag_print ist (Proofview_monad.Info.Builtin (Pp.str "nat_or_var")) (
        Proofview.Trace.new_deferred_placeholder >>= fun deferred_id ->
        Ftactic.return (Proofview.Tagged.make deferred_id v)
      )
    )
