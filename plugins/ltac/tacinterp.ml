...
(* "_f" suffix means that the function returns [_ Ftactic.t], as opposed to [_ Proofview.tactic]. *)
...
let rec val_interp_f ist ?(appl=UnnamedAppl) (tac:glob_tactic_expr) : Val.t Ftactic.t =
...
and force_vrec ist v : Val.t Ftactic.t =
  if has_type v (topwit wit_tacvalue) then
    ...
    match v with
    | VRec (lfun,body) -> val_interp_f {ist with lfun = !lfun} body
    | v -> Ftactic.return (of_tacvalue v)
  else Ftactic.return v

and interp_ltac_reference_f ?loc' mustbetac ist r : Val.t Ftactic.t =
  match r with
  | ArgVar {loc;v=id} ->
      let (>>=) = Ftactic.bind in
      let v =
  ...
  | ArgArg (loc,r) ->
      ...
      do_profile stack ~count_call:false
        (catch_error_tac_loc (* loc for interpretation *) loc stack
           (val_interp_f ~appl ist (Tacenv.interp_ltac r)))

and interp_tacarg ist arg : Val.t Ftactic.t =
  match arg with
  | TacGeneric (_,arg) -> interp_genarg ist arg
  | Reference r -> interp_ltac_reference_f false ist r
  | ConstrMayEval c ->
  ...
  | TacCall { v=(r,[]) } ->
      interp_ltac_reference_f true ist r
  | TacCall { loc; v=(f,l) } ->
      let (>>=) = Ftactic.bind in
      interp_ltac_reference_f true ist f >>= fun fv ->
      ...
      if List.is_empty lvar then
        begin wrap_error
            begin
              ...
              let (stack, _) = trace in
              do_profile stack ~count_call:false
                (catch_error_tac_loc loc stack (val_interp_f (ensure_loc loc ist) body)) >>= fun v ->
...
and interp_match ist lz constr lmr =
  let (>>=) = Ftactic.bind in
  begin wrap_error
    (interp_ltac_constr_f ist constr)
    begin function
      | (e, info) ->
...
and interp_ltac_constr_f ist e : EConstr.t Ftactic.t =
  let (>>=) = Ftactic.bind in
  begin wrap_error
      (val_interp_f ist e)
