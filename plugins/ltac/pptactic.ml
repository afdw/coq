...
let pr_goal_selector ~toplevel s =
  ...
  type 'a printer = {
    ...
    pr_deref_arg : 'a gen_tactic_arg -> 'a gen_tactic_arg;
  }
    ...
    let rec prtac n (t:glob_tactic_expr) =
      let pr = {
        ...
        pr_deref_arg = (fun x ->
          match x with
            | TacGeneric (isquot, g) -> (
              let g = !current_dereffer.deref env sigma None g in
              if Genarg.has_type g (Glbwit wit_tactic) then
                match out_gen (Glbwit wit_tactic) g with
                | {CAst.v = TacArg y} -> y
                | _ -> TacGeneric (isquot, g)
              else
                TacGeneric (isquot, g)
            )
            | _ -> x
