    ...
    | LocalAssum (name, type_) ->
      {
        ...
        type_ = print_constr env sigma type_;
        kind = Assumption;
      }
    | LocalDef (name, value, type_) ->
      {
        name = name.binder_name |> Names.Id.to_string;
        type_ = print_constr env sigma type_;
        kind = Definition {value = print_constr env sigma value};
      }
end
...
module Goal = struct
  ...
  let make sigma g =
    ...
    in
    {
      ...
      concl = print_constr env sigma concl;
    }
end
...
module Step = struct
  type kind =
    | Tactic of {
        ...
        event : Constrextern.PrintingVariants.t Proofview_monad.Event.t;
      }
    | StartSubproof
    | EndSubproof
...
let record_step proof_before proof_after kind =
  ...
  current_steps := !current_steps @ [Step.{
    goals_before = goals_before |> List.map (Goal.make sigma_before);
    goals_after = goals_after |> List.map (Goal.make sigma_after);
    kind = kind;
  }]
