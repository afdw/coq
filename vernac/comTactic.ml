...
let solve ~pstate n ~info ~print t ~with_end_tac =
  Proof.printed_goal_selector := None;
  ...
  if !Flags.tracing && !Proof.printed_root_tactic <> None then
    ComDefinition.record_step (pstate |> Declare.Proof.get) (p |> Declare.Proof.get) (ComDefinition.Step.Tactic {
      goal_selector = !Proof.printed_goal_selector |> Option.get;
      tactic_raw = print () |> Pp.simple_string_of_ppcmds;
  ...
  if !Flags.tracing_interactive && !Proof.printed_root_tactic <> None then begin
    ...
    Feedback.msg_info Pp.(str "Event:" ++ spc () ++ str (!Proof.event |> Option.get |>
      Proof.Event.to_yojson |> Yojson.Safe.pretty_to_string))
  end;
  p
