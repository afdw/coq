...
let interp_qed_delayed_proof ~proof ~st ~control (CAst.{loc; v = pe } as e) : Vernacstate.Interp.t =
  ...
  Vernacentries.end_proof
    ~name:(proof |> Declare.Proof.get_po_name)
    ~outcome:(match pe with Admitted -> Vernacentries.Theorem.Admitted | Proved _ -> Vernacentries.Theorem.Proved);
