...
module Info = struct
  ...
  let empty_saved_proofview = (Evd.empty, [])
    ...
    | Node (TagDeferredContents deferred_id, f) ->
      ...
      if not (deferred_map |> DeferredIdMap.mem deferred_id) then
        ...
        deferred_map |> DeferredIdMap.add deferred_id [
          Node (
            TagDispatch empty_saved_proofview,
            [
              Node (
                TagDispatchBranch,
                [
                  Node (
                    TagTactic (empty_saved_proofview, ref empty_saved_proofview, Primitive (Pp.str "<error>"), fun () -> Pp.(str "Duplicate contents for deferred_id " ++ int deferred_id)),
                    []
                  )
                ]
              );
  ...
  let rec substitute_deferred_tree deferred_map = function
    ...
    | Node (TagDeferredPlaceholder deferred_id, []) ->
      ...
      deferred_map
        |> DeferredIdMap.find_opt deferred_id
        |> Option.map (substitute_deferred_forest deferred_map)
        |> Option.default [Node (TagTactic (empty_saved_proofview, ref empty_saved_proofview, Primitive (Pp.str "<error>"), fun () -> Pp.(str "No contents for deferred_id " ++ int deferred_id)), [])]
  ...
  let rec convert_tree = function
    ...
    | Node (TagDispatch saved_proofview_before, f) ->
      Dispatch (
        saved_proofview_before,
        f |> List.map (function
    ...
    | Node (TagTactic (saved_proofview_before, saved_proofview_after, kind, m), f) -> Tactic (saved_proofview_before, !saved_proofview_after, kind, m, convert_forest f)
  ...
  let rec compress = function
    | Sequence brs ->
      ...
      | [c] -> c
      | brs -> Sequence brs)
    | Dispatch (_saved_proofview_before, [c]) -> compress c
    | Dispatch (saved_proofview_before, brs) ->
      let brs = brs |> List.map compress in
      let all_empty = brs |> List.for_all (fun e -> match e with Sequence [] -> true | _ -> false) in
      if all_empty then Sequence [] else Dispatch (saved_proofview_before, brs)
    | Tactic (saved_proofview_before, saved_proofview_after, kind, m, c) -> Tactic (saved_proofview_before, saved_proofview_after, kind, m, compress c)
    | Message m -> Message m
  ...
  let rec print = let open Pp in function
    ...
    | Dispatch (_saved_proofview_before, brs) ->
      let sep () = spc () ++ str "|" ++ spc () in
      if brs = []
      then str "idtac"
      else str "[>" ++ spc () ++ (brs |> Pp.prlist_with_sep sep print) ++ spc () ++ str "]"
    | Tactic (_saved_proofview_before, _saved_proofview_after, kind, m, _) -> m ()
    | Message m -> str "(* " ++ m () ++ str " *)"
