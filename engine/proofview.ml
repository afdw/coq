...
let apply ~name ~poly env t sp =
  ...
  | Cons ((r, (state, _), status, pretrace), _) ->
    let trace = Info.compress (Info.apply_contexts (Info.give_contexts (Info.finalize pretrace))) in
...
module Trace = struct
  ...
  let tag_dispatch f = InfoL.tag (Info.Dispatch []) (f (InfoL.tag (Info.DispatchBranch (Info.Seq []))))
  let message m = InfoL.leaf (Info.Message (fun (env, sigma) -> m env sigma))
  let tag_tactic m t = InfoL.tag (Info.Tactic ((fun (env, sigma) -> m env sigma), Info.Seq [])) t
  let tag_body t = InfoL.tag (Info.Body (Info.Seq [])) t
  let tag_barrier t = InfoL.tag (Info.Barrier (Info.Seq [])) t
  let tag_context context t = InfoL.tag (Info.Context (context, Info.Seq [])) t
...
let shelve =
  let open Proof in
  Trace.tag_tactic (fun _ _ -> Pp.str "shelve") (
    Comb.get >>= fun initial ->
    Comb.set [] >>
...
let shelve_goals l =
  let open Proof in
  Trace.tag_tactic (fun _ _ -> Pp.str "shelve_goals") (
    Comb.get >>= fun initial ->
...
let shelve_unifiable_informative =
  let open Proof in
  Trace.tag_tactic (fun _ _ -> Pp.str "shelve_unifiable") (
    Pv.get >>= fun initial ->
...
let cycle n =
  Trace.tag_tactic (fun _ _ -> Pp.(str "cycle" ++ spc () ++ int n)) (
...
let swap i j =
  Trace.tag_tactic (fun _ _ -> Pp.(str "swap" ++ spc () ++ int i ++ spc () ++ int j)) (
    Comb.modify (fun initial ->
      let l = CList.length initial in
...
let revgoals =
  Trace.tag_tactic (fun _ _ -> Pp.str "revgoals") (Comb.modify CList.rev)
...
let give_up =
  let open Proof in
  Trace.tag_tactic (fun _ _ -> Pp.str "give_up") (
    Comb.get >>= fun initial ->
    Comb.set [] >>
    mark_as_unsafe >>
  ...
  let enter f =
    Trace.tag_dispatch (fun tag_branch ->
      iter_goal (fun goal ->
        ...
        try tag_branch (Trace.tag_context (gl.env, gl.sigma) (f gl))
