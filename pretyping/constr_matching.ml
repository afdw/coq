...
let instantiate_pattern env sigma lvar c =
  ...
  | PVar id as x ->
      (try
        let ctx,c = Id.TracedMap.find id lvar in
        try
          let inst =
            List.map
...
let extended_matches env sigma pat c =
  ...
  names, subst |> Id.TracedMap.remember

let matches env sigma pat c =
  snd (matches_core_closed env sigma (Id.Set.empty,pat) c) |> Id.TracedMap.remember
...
let authorized_occ env sigma closed pat c mk_ctx =
  try
    ...
    then (fun next -> next ())
    else (fun next -> mkresult (fst subst, snd subst |> Id.TracedMap.remember) (lazy (mk_ctx (mkMeta special_meta))) next)
