...
let detype_closed_glob ?isgoal avoid env sigma t =
  let convert_id cl id =
    try Id.ObservableMap.find id cl.idents
    with Not_found -> id
  in
  ...
  let rec detype_closed_glob cl cg : Glob_term.glob_constr = DAst.map (function
    | GVar id ->
        ...
        begin try
          GVar(Id.ObservableMap.find id cl.idents)
        (* if [id] is bound to a typed term *)
        with Not_found -> try
          (* assumes [detype] does not raise [Not_found] exceptions *)
          let (b,c) = Id.ObservableMap.find id cl.typed in
        ...
        with Not_found -> try
          let {closure;term} = Id.ObservableMap.find id cl.untyped in
