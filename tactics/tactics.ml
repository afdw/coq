  ...
  Proofview.Goal.enter begin fun gl ->
    ...
    | LocalHypConv ->
      ...
      let fold accu arg =
        ...
        let old = try Id.ObservableMap.find id accu with Not_found -> [] in
        Id.ObservableMap.add id (redfun :: old) accu
      in
      let reds = List.fold_left fold Id.Map.empty args in
      let evdref = ref sigma in
      let map d =
        let id = NamedDecl.get_id d in
        match Id.ObservableMap.find id reds with
        | reds ->
...
let make_change_arg c pats env sigma = (sigma, replace_vars sigma (Id.Map.bindings (pats |> Id.ObservableMap.mark_all)) c)
...
let reduce redexp cl =
  ...
  Proofview.Trace.tag_tactic (Proofview_monad.Info.Primitive (Pp.str "reduce")) (fun () -> trace env sigma) begin
