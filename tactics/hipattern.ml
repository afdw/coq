...
let match_arrow_pattern env sigma t =
  ...
  match Id.Map.bindings result with
    | [(m1,arg);(m2,mind)] ->
      assert (Id.equal m1 meta1 && Id.equal m2 meta2); (arg |> Id.ObservableMap.mark, mind |> Id.ObservableMap.mark)
...
let match_sigma env sigma t =
  match Id.Map.bindings (matches env sigma (Lazy.force coq_sig_pattern) t) with
    | [(_,a); (_,p)] -> (a |> Id.ObservableMap.mark,p |> Id.ObservableMap.mark)
...
let match_eqdec env sigma t =
  ...
  | [(_,typ);(_,c1);(_,c2)] ->
      eqonleft, lib_ref op, c1 |> Id.ObservableMap.mark, c2 |> Id.ObservableMap.mark, typ |> Id.ObservableMap.mark
  | _ -> anomaly (Pp.str "Unexpected pattern.")
