...
let rec pr_closure {idents=idents;typed=typed;untyped=untyped} =
  hov 1 (str"{idents=" ++ prididmap (idents |> Id.TracedMap.forget) ++ str";" ++ spc() ++
         str"typed=" ++ prconstrunderbindersidmap (typed |> Id.TracedMap.forget) ++ str";" ++ spc() ++
         str"untyped=" ++ pr_closed_glob_constr_idmap (untyped |> Id.TracedMap.forget) ++ str"}")
...
let ppist ist =
  ...
  pp (pridmap pr (ist.Geninterp.lfun |> Id.TracedMap.forget))
