...
module Trace = struct
  type t = {
    ...
    declarations : ComDefinition.Declaration.t list;
...
let start_coq custom =
  ...
  at_exit (fun () ->
    ...
    if !Flags.tracing_interactive then
      Feedback.msg_info Pp.(str "Collected declarations:" ++ spc () ++ int (!ComDefinition.declarations |> List.length));
      ...
      Util.modify_file_with_lock ~filename ~f:(fun old_contents ->
        ...
        let trace =
          if !Flags.tracing_split then
            ...
            let sub_trace = {
              Trace.sub_filenames = [];
              Trace.declarations = !ComDefinition.declarations;
            } in
            ...
            {
              trace with
              Trace.declarations = trace.declarations @ !ComDefinition.declarations;
            } in
