...
let q_infinity () =
  try
    qualid_of_ref "num.float.infinity"
  with e when CErrors.noncritical e ->
    Libnames.make_qualid DirPath.empty (Id.of_string "infinity")
let q_neg_infinity () =
  try
    qualid_of_ref "num.float.neg_infinity"
  with e when CErrors.noncritical e ->
    Libnames.make_qualid DirPath.empty (Id.of_string "neg_infinity")
let q_nan () =
  try
    qualid_of_ref "num.float.nan"
  with e when CErrors.noncritical e ->
    Libnames.make_qualid DirPath.empty (Id.of_string "nan")
